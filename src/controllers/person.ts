...
const patchPerson = async ({ params, body }: Request, res: Response) => {
    const { id } = params;
    const { userData, personData } = body;
    const response = await updatePerson(id, userData, personData);
    if (response === 'USER_NOT_FOUND') return  httpResponse.NotFound(res, response);
    httpResponse.Ok(res, response);
}
...
const createMember = async ({ body }: Request, res: Response) => {
    const { userData, personData } = body;
    const response = await createNewMember(userData, personData);
    httpResponse.Ok(res, response);
}
